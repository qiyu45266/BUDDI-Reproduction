...
2. **Drop full-from-scratch training** (decided 2026-05-18). The FlickrCI3D
   ...
   `processed_pseudogt_fits.pkl` would require ~30 GPU-h of `flickr_fits.yaml`
   re-fitting that we cannot afford. The upstream maintainer has not responded
...
- All artifacts under `repro/phase_a/outputs/`; large `.pkl` / `.gif` files
  gitignored but regenerable via `repro/phase_a/runners/run_uncond.sh`
